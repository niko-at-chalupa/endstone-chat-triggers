...
class TwitchApiClient:
        ...
        if self._websocket is not None:
            try:
                future = asyncio.run_coroutine_threadsafe(
                    self._websocket.stop(), self._loop
                )
    ...
    async def _async_main(self):
        try:
            ...
            await self._websocket.listen_channel_follow_v2(
                user_id, user_id, self._on_channel_follow
            )
            await self._websocket.listen_channel_subscribe(
                user_id, self._on_channel_subscribe
            )
            await self._websocket.listen_channel_subscription_gift(
                user_id, self._on_channel_subscription_gift
            )
            await self._websocket.listen_channel_subscription_message(
                user_id, self._on_channel_subscription_message
            )
            await self._websocket.listen_channel_cheer(user_id, self._on_channel_cheer)
            await self._websocket.listen_channel_raid(
                to_broadcaster_user_id=user_id, callback=self._on_channel_raid
            )
            await self._websocket.listen_channel_points_custom_reward_redemption_add(
                user_id, self._on_channel_points_redemption
            )
            await self._websocket.listen_channel_prediction_begin(
                user_id, self._on_prediction_begin
            )
            await self._websocket.listen_channel_prediction_progress(
                user_id, self._on_prediction_progress
            )
            await self._websocket.listen_channel_prediction_end(
                user_id, self._on_prediction_end
            )

            self._logger.info(
                f"TwitchAPI client connected and listening events for {user_id}"
            )
            while self._running:
    ...
    async def _on_channel_subscription_message(
        self, event: ChannelSubscriptionMessageEvent
    ):
        if self._twitch is None:
            return
        data = event.event
    ...
    async def _on_channel_points_redemption(
        self, event: ChannelPointsCustomRewardRedemptionAddEvent
    ):
        if self._twitch is None:
            return
        data = event.event
